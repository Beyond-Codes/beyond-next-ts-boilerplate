...
function ConfirmPopup(): JSX.Element | null {
  ...
  return (confirmPopup?.inHTML && (
    <div
      className={`${containerClass} fixed left-0 top-0 z-[9999] flex h-full w-full items-center justify-center overflow-hidden bg-black/70 p-5 font-theme transition-all delay-200 duration-300`}
    >
      <div
        className={`${popupClass}
        grid max-h-fit w-full max-w-md grid-cols-1 place-content-start place-items-center gap-10 rounded-lg border border-zinc-800 bg-zinc-900 p-5 shadow-xl transition-all duration-300`}
      >
        <Icon iconName="info" />
        <p
          key={popupText}
        ...
        <div className="flex w-full items-center justify-center gap-5">
          ...
          <button
            className="min-w-[90px] rounded-md bg-red-400/10 px-4 py-1 text-center text-sm text-red-400 transition-all duration-150 hover:bg-red-400/20 disabled:pointer-events-none disabled:opacity-50"
            type="button"
